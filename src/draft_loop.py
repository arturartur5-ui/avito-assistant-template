...
def handle_callback(client: AvitoBusinessClient, conn: sqlite3.Connection, cb: dict) -> None:
    ...
    if OWNER_CHAT and who != str(OWNER_CHAT) and str(chat) != str(OWNER_CHAT):
...
def handle_message(client: AvitoBusinessClient, conn: sqlite3.Connection, msg: dict) -> None:
    ...
    if str(chat) != str(OWNER_CHAT):
        return                                      # бот отвечает только владельцу
    draft_id = AWAITING_EDIT.pop(chat, None)
    text = (msg.get("text") or "").strip()
    if draft_id is None or not text:
        return
