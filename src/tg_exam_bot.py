...
def handle_message(eng: Engine, msg: dict) -> None:
    ...
    if a.failed:
        ...
        head = ("⚠️ Запрос не отправлен: своя проверка нашла в тексте похожее на личные данные.\n"
                if "персональные данные" in a.reason else "⚠️ Шлюз не ответил, помощник промолчал.\n")
        send(chat_id, head + a.reason +
