...
NOISE_PATTERNS = [
    ...
    re.compile(r"</?[A-Z][A-Za-z]*[^>]*>"),          # <TldrSection>, <Callout …>
    ...
    # таблиц («Рыночные программы», «Стоимость услуг») переставали быть
...
def build_all(knowledge: Path, corpus: Path) -> list[Chunk]:
    ...
    for p in sorted((knowledge / "articles").glob("*.md")) if (knowledge / "articles").exists() else []:
