...
class NotificationIsChecked(unittest.TestCase):
    """Уведомление специалисту идёт через ту же проверку, что и дверь к модели."""

    def test_leaky_text_is_not_forwarded(self):
        out = io.StringIO()
        with contextlib.redirect_stdout(out):
            draft_loop.notify(1, "u2i-1", "мой вотсап 8#900#000#00#00", "Ок", False, "", dry_run=True)
        text = out.getvalue()
        self.assertNotIn("8#900", text)
        self.assertIn("откройте чат в Авито", text)

    def test_send_button_from_stranger_is_ignored(self):
        """Групповой DRAFT_OWNER_CHAT: кнопку «отправить» жмёт посторонний участник.

        Условие допуска стояло через «и», поэтому совпадения чата хватало —
        сообщение уходило живому клиенту от чужого нажатия.
        """
        conn = memory_db(); avito = FakeAvito()
        did = add_draft(conn, "Здравствуйте! Расскажите, что нужно сделать?")
        draft_loop.OWNER_CHAT = "-1001234567890"
        sent_replies = []
        real_tg = draft_loop.tg
        draft_loop.tg = lambda method, **kw: sent_replies.append((method, kw))
        try:
            draft_loop.handle_callback(avito, conn, {
                "id": "1", "data": f"send:{did}", "from": {"id": 999999},
                "message": {"chat": {"id": -1001234567890}}})
        finally:
            draft_loop.tg = real_tg; draft_loop.OWNER_CHAT = ""
        self.assertEqual(avito.sent, [], "постороннее нажатие ушло клиенту")
        self.assertEqual(conn.execute("SELECT status FROM drafts WHERE id=?", (did,)).fetchone()[0], "wait")

    def test_edit_from_stranger_is_ignored(self):
        """В группе чужое сообщение после «поправить» не должно уйти клиенту."""
        conn = memory_db(); avito = FakeAvito()
        did = add_draft(conn, "заготовка")
        draft_loop.OWNER_CHAT = "100"
        draft_loop.AWAITING_EDIT[100] = did
        draft_loop.handle_message(avito, conn, {"chat": {"id": 100}, "from": {"id": 999}, "text": "Мой ответ клиенту"})
        self.assertEqual(avito.sent, [], "чужая правка ушла клиенту")
        draft_loop.AWAITING_EDIT.clear(); draft_loop.OWNER_CHAT = ""
