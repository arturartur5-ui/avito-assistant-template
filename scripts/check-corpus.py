...
sys.path.insert(0, str(ROOT))          # config.niche: слова ниши, которые не имена
...
def scan(paths: list[Path]) -> tuple[Counter, list[str], int]:
    ...
    for path in paths:
        ...
        for f in targets:
            if f.suffix.lower() not in TEXT_SUFFIX or f.name.startswith(".") or ".template." in f.name:
                continue
            files += 1
...
def main() -> int:
    ...
    print("\nЧто делать:\n"
          ...
          "  2. Если это настоящие данные — покажите строку (без значения) разработчику\n"
          "     или агенту: «в обезличивании пропускается такая-то форма». Правило\n"
          "     дописывается в src/anonymize.py вместе с тестом; сами вы это не делаете.\n"
          "  3. Если это термин вашей ниши, принятый за имя, — добавьте слово в NOT_NAMES\n"
          "     или GEO_WORDS в config/niche.py, раздел 13.\n"
          "  4. Соберите корпус заново — правка правил задним числом не чинит\n"
