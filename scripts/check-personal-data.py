...
def terms_state() -> str:
    """Почему список терминов пуст. Случаев три, и чинятся они по-разному,
    поэтому сообщение обязано их различать: человеку, который только скачал
    шаблон, «нет терминов» ничего не говорит."""
    if not TERMS_FILE.exists():
        return f"файла {TERMS_FILE.name} нет"
    try:
        lines = [s.strip() for s in
                 TERMS_FILE.read_text(encoding="utf-8").splitlines() if s.strip()]
    except OSError as e:
        return f"{TERMS_FILE.name} не прочитать — {e}"
    if not lines:
        return f"{TERMS_FILE.name} пустой"
    return f"в {TERMS_FILE.name} одни комментарии"
